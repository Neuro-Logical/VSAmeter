...
def datadir_to_lhotse_with_alis(
    data_dir: str,
    ali_dir: str,
    output_dir: str,
    unit: str,
    ...
    It reads DATA_DIR, and converts it to Lhotse RecordingSet and SupervisionSet manifests.
    Then it stores the alignments in HDF5, and adds the fields "storage_type", "storage_key",
    and "storage_path" to the supervisions.

    The manifests with alignments are stored in OUTPUT_DIR together with the HDF5
    archive and a phones.txt file indicating the int2sym mapping.
    """
    # Set up path operations, read input files
    ali_dir = Path(ali_dir)
    output_dir = Path(output_dir)
    output_dir.mkdir(exist_ok=True, parents=True)
    # Omit frame_shift arg as we don't need Kaldi MFCC features.
    recordings, supervisions, _ = load_kaldi_data_dir(data_dir, sampling_rate=sampling_rate)
